...
class StaticModelTwoBoxEst(nn.Module):
    ...
    def forward(self, pts, init_box, bbox_gt):
        ...
        center_boxnet_one, heading_scores_one, heading_residuals_normalized_one, heading_residuals_one, \
        size_scores_one, size_residuals_normalized_one, size_residuals_one = parse_output_to_tensors(box_pred_one, logits, mask)
        center_one = center_boxnet_one + init_box[:, :3] # (bs, 3)
        ...
        object_pts_xyz_two = copy.deepcopy(object_pts_xyz) # (bs, 3, NUM_OBJECT_POINT)
        ...
        center_boxnet_two, heading_scores_two, heading_residuals_normalized_two, heading_residuals_two, \
        size_scores_two, size_residuals_normalized_two, size_residuals_two = parse_output_to_tensors(box_pred_two, logits, mask)
        center_two = center_boxnet_two + center_one # (bs, 3)
        
        output = {
            'logits': logits,
            'mask': mask,
            'center_boxnet_one': center_boxnet_one,
            ...
            'center_boxnet_two': center_boxnet_two,
...
class STATICTRACK(Dataset):
    ...
    def __getitem__(self, index):
        ...
        heading_class_label, heading_residual_label = angle2class(bbox_gt[-1] - bbox[0, -1], NUM_HEADING_BIN)
